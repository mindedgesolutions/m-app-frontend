...
const AppListSubcategories = () => {
  document.title = `Sub-Category List | ${titles.siteName}`;
  const [data, setData] = useState<SubCategoryProps[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await customFetch.get('/admin/sub-categories', {
  ...
  return (
    <div>
      <AppTitleWrapper title="sub-category list" description={description} />
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="col-span-1 order-2 md:order-1 md:col-span-2">
          <AppSubcategorySearch
          ...
          <Table className="text-xs font-inter">
            ...
            <AppTableBody loading={loading} colSpan={5} data={data}>
              {data.map((category, index) => (
                <TableRow key={category.id}>
                  ...
                  <TableCell>
                    <AppTooltip text={category.category_name} />
                  </TableCell>
                  <TableCell>
                    <div className="flex">
        ...
        <div className="col-span-1 order-1 md:order-2">
          <AppAddEditSubcategory editId={editId} setEditId={setEditId} />
        </div>
      </div>
    </div>
  );
};
export default AppListSubcategories;
